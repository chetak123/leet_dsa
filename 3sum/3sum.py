class Solution:
    def threeSum(self, nums: List[int]) -> List[List[int]]:
        """The way to think about it is since it's 3 sum, there's only going to be 3 numbers. So to find the combinations of 3 numbers, it is iterating through the 
        list with the first pointer, and then trying to find two extra numbers to sum to 0. Since the list is ordered, 
        the right pointer will always be higher than the middle pointer. So if the sum is too large, you can move the right pointer back one. 
...
if left > 0 and nums[left] == nums[left-1] is because when left = 0, it doesn't need to check if it's a duplicate element since it doesn't even have a previous 
element to compare with. And nums[left] == nums[left-1] is to prevent checking duplicate again. 
